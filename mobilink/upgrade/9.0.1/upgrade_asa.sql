...
create trigger ml_connection_script_trigger 
    after insert, update, delete on ml_connection_script
    for each row
begin
...
create trigger ml_script_trigger 
    after insert, update, delete on ml_script
    for each row
begin
    update ml_scripts_modified set last_modified = dateformat( CURRENT TIMESTAMP, 'yyyy/mm/dd hh:nn:ss' );
end
...
create trigger ml_table_script_trigger 
    after insert, update, delete on ml_table_script
    for each row
begin
    update ml_scripts_modified set last_modified = dateformat( CURRENT TIMESTAMP, 'yyyy/mm/dd hh:nn:ss' );
end
...
drop procedure ml_upload_update_device_address
go
