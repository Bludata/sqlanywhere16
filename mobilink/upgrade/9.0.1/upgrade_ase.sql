...
create table #ml_subscription_temp (
    ...
    last_upload_time	    datetime	    default '1900-01-01 00:00:00' not null,
    last_download_time	    datetime	    default '1900-01-01 00:00:00' not null )
go
...
insert into #ml_subscription_temp ( rid, subscription_id, user_id,
    ...
    select user_id, subscription_id, user_id, progress,
...
--
-- Drop the old ml_user and ml_subscription tables.
--
...
SET IDENTITY_INSERT ml_user ON
go
...
SET IDENTITY_INSERT ml_user OFF
go
...
create table ml_database (
    ...
    remote_id		varchar( 128 )	not null unique,
    script_ldt		datetime	default '1900-01-01 00:00:00' not null,
...
SET IDENTITY_INSERT ml_database OFF
go
...
drop trigger ml_script_trigger
go

drop trigger ml_table_script_trigger
go

drop trigger ml_connection_script_trigger
go

--
-- Add checksum column to ml_script
--
...
create table ml_column (
    ...
    table_id	integer		not null references ml_table ( table_id ),
...
create procedure ml_add_passthrough_script
    ...
    @flags			varchar( 256 ),
    @affected_pubs		varchar( 16384 ),
    @script			varchar( 16384 ),
...
go
exec sp_procxmode 'ml_add_passthrough_script', 'anymode'
go
...
go
exec sp_procxmode 'ml_delete_passthrough_script', 'anymode'
go
...
as
	...
	if @script_name is not null
	    ...
	    select @name = 'NULL'
	select @msg = 'Passthrough script name: "' + @name +
			'" does not exist in the ml_passthrough_script table.'
	raiserror 20000 @msg
    end 
go
exec sp_procxmode 'ml_add_passthrough', 'anymode'
go
...
go
exec sp_procxmode 'ml_delete_passthrough', 'anymode'
go
...
as
	...
	if @action in ( 'R', 'S', 'P', 'H', 'r', 's', 'p', 'h' ) begin
	    ...
	    select @msg = 'Invalid action: "' + @action + '".'
	    raiserror 20000 @msg
	end
    end
    else begin
	select @msg = 'Invalid failed_script_name: "' + @action + '".'
	raiserror 20000 @msg
    end
go
exec sp_procxmode 'ml_add_passthrough_repair', 'anymode'
go
...
go
exec sp_procxmode 'ml_delete_passthrough_repair', 'anymode'
go

create trigger ml_script_trigger on ml_script for insert, update, delete
as
    update ml_scripts_modified set last_modified = getdate()
go

create trigger ml_table_script_trigger on ml_table_script for insert, update, delete 
as
    update ml_scripts_modified set last_modified = getdate()
go	

create trigger ml_connection_script_trigger on ml_connection_script for insert, update, delete
as
    update ml_scripts_modified set last_modified = getdate()
go
...
--
-- Restructure the ml_property table.
--

update ml_property
set property_value = 'ROGERS'
where component_name = 'SIS' and
    property_set_name = 'Carrier(Rogers)' and
    property_name = 'network_provider_id' and
    convert( varchar(512), property_value ) = 'ROGERS AT&T'
go
...
as
    ...
    if @script is not null begin
    	...
    	select @upd_script_id = script_id from ml_table_script
...
go
exec sp_procxmode 'ml_add_lang_table_script_chk',	   'anymode'
go
...
go
exec sp_procxmode 'ml_add_lang_conn_script_chk', 'anymode'
go
...
--
-- Replace the ml_add_dnet_... scripts
--
...
go
exec sp_procxmode 'ml_add_property',		   'anymode'
go
...
go
exec sp_procxmode 'ml_add_column',	   'anymode'
go
...
--
-- The ml_user column in ml_listening table has been renamed to "name"
-- ml_device char(1)->varchar2(1) and ml_device_address DATE->Timestamp
--
create table #ml_listening_temp (
    name		varchar(128)	not null,
...
insert into #ml_listening_temp(name,device_name,listening,ignore_tracking,source)
    select ml_user,device_name,listening,ignore_tracking,source
        from ml_listening
go
...
create table #ml_device_address_temp (
    device_name		varchar(255)	not null,
...
insert into #ml_device_address_temp(device_name,medium,address,active,last_modified,ignore_tracking,source)
    select device_name,medium,address,active,last_modified,ignore_tracking,source
	from ml_device_address
go

drop table ml_device_address
go

create table #ml_device_temp (
    device_name		varchar(255)	not null,
...
insert into #ml_device_temp(device_name,listener_version,listener_protocol,info,ignore_tracking,source)
    select device_name,listener_version,listener_protocol,info,ignore_tracking,source
	from ml_device
go
...
insert into ml_device(device_name,listener_version,listener_protocol,info,ignore_tracking,source)
    select device_name,listener_version,listener_protocol,info,ignore_tracking,source from #ml_device_temp
go

drop table #ml_device_temp
go
...
insert into ml_listening(name,device_name,listening,ignore_tracking,source)
    select name,device_name,listening,ignore_tracking,source
	from #ml_listening_temp
go
...
insert into ml_device_address(device_name,medium,address,active,last_modified,ignore_tracking,source)
    select device_name,medium,address,active,last_modified,ignore_tracking,source from #ml_device_address_temp
go

drop table #ml_device_address_temp
go
...
as
    ...
    select @d = device_name from ml_listening
	    where name = @name
    if @d is null begin
	insert into ml_listening(name, device_name, listening, ignore_tracking, source )
...
go
exec sp_procxmode 'ml_set_listening',		   'anymode'
go
...
as
	...
	set @lut = ( select last_upload from ml_sis_sync_state where remote_id = @remote_id and subscription_id = @sid )
...
go
exec sp_procxmode 'ml_set_sis_sync_state',		    'anymode'
go
...
go
exec sp_procxmode 'ml_upload_update_listening',		   'anymode'
go
...
go
exec sp_procxmode 'ml_delete_listening',	   'anymode'
go
...
go
exec sp_procxmode 'ml_set_device',		   'anymode'
go

create procedure ml_upload_upd_device_address
    @address		varchar( 255 ),
    @active		varchar( 1 ),
    @device		varchar( 255 ),
    @medium		varchar( 255 )
...
go
exec sp_procxmode 'ml_upload_upd_device_address',	   'anymode'
go
...
drop procedure ml_qa_stage_msg_for_server
go
drop procedure ml_qa_staged_msg_for_client
go
