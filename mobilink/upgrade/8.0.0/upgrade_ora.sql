...
-- In 8.0.0, publication_name actually stores the
-- subscription_id sent by the client
insert into ml_subscription_temp ( rid, user_id,
				   subscription_id, progress )
    select user_id, user_id, publication_name, progress
...
--
-- Using CLOB instead of LONG
--
alter table ml_connection_script drop constraint ml_c_version
/

alter table ml_connection_script drop constraint ml_c_script
/

alter table ml_table_script drop constraint ml_t_version
/

alter table ml_table_script drop constraint ml_t_table
/

alter table ml_table_script drop constraint ml_t_script
/
...
alter table ml_script rename to ml_script_temp
/

create table ml_script (
    script_id		integer			not null,
    script		clob    		not null,
    script_language	varchar2( 128 )		default 'sql' not null
    ,
...
insert into ml_script ( script_id, script, script_language )
  select script_id, TO_LOB(script), script_language from ml_script_temp
/


drop table ml_script_temp
/

alter table ml_script_version rename to ml_script_version_temp
/
...
insert into ml_script_version ( version_id, name, description )
  select  version_id, name, TO_LOB( description ) from ml_script_version_temp
/

drop table ml_script_version_temp
/

--
-- Create the ml_column table and the ml_add_column procedure.
--
create table ml_column (
    ...
    constraint ml_col_version foreign key( version_id ) references ml_script_version,
    constraint ml_col_table foreign key( table_id ) references ml_table,
...
alter table ml_connection_script add constraint ml_c_version 
foreign key ( version_id ) references ml_script_version
/

alter table ml_connection_script add constraint ml_c_script 
foreign key ( script_id ) references ml_script
/

alter table ml_table_script add constraint ml_t_version 
foreign key ( version_id ) references ml_script_version
/

alter table ml_table_script add constraint ml_t_table 
foreign key ( table_id ) references ml_table
/

alter table ml_table_script add constraint ml_t_script 
foreign key ( script_id ) references ml_script
/
...
/
insert into ml_scripts_modified ( last_modified ) values ( SYSTIMESTAMP)
/

create trigger ml_script_trigger after insert or update or delete on ml_script
    for each row
begin
...
create trigger ml_table_script_trigger after insert or update or delete on ml_table_script
    for each row
begin
    update ml_scripts_modified set last_modified = SYSTIMESTAMP ;
end;
...
create trigger ml_connection_script_trigger after insert or update or delete on ml_connection_script
    for each row
begin
    update ml_scripts_modified set last_modified = SYSTIMESTAMP ;
end;
...
begin
	...
	if v_table_id is null then
	    ...
	    insert into ml_table ( table_id, name ) values ( v_table_id, p_table );
	end if;	    	
...
as
begin
    ml_add_lang_table_script_chk( p_version, p_table, p_event, p_script_language, p_script, null );
end;
/
...
as
begin
    ml_add_lang_table_script( p_version, p_table, p_event, 'sql', p_script );
end;
/
...
as
begin
    ml_add_lang_table_script( p_version, p_table, p_event, 'java', p_script );
end;
/
...
as
begin
    ml_add_lang_table_script( p_version, p_table, p_event, 'dnet', p_script );
end;
/
...
begin
    ...
    if p_script is not null then
	if v_version_id is null then
	    -- Insert into the ml_script_version
	    select max( version_id )+1 into v_version_id from ml_script_version;
	    if v_version_id is null then
		v_version_id := 1;
	    end if;
...
as
begin
    ml_add_lang_conn_script_chk( p_version, p_event, p_script_language, p_script, null );
end;
/
...
as
begin
    ml_add_lang_connection_script( p_version, p_event, 'sql', p_script );
end;
/
...
as
begin
    ml_add_lang_connection_script( p_version, p_event, 'java', p_script );
end;
/
...
as
begin
    ml_add_lang_connection_script( p_version, p_event, 'dnet', p_script );
end;
/
...
create table ml_property (
    ...
    property_set_name	varchar2( 128 )		not null,
...
create table ml_device (
    ...
    listener_version	varchar2( 128 )	not null,
    listener_protocol	integer		not null,
    info		varchar2( 255 )	not null,
    ignore_tracking	varchar2( 1 )	not null,
...
create table ml_device_address (
    ...
    constraint ml_da_dn foreign key( device_name ) references ml_device ( device_name )
)
/
...
create table ml_listening (
    name		varchar2( 128 )	not null,
    device_name		varchar2( 255 )	not null,
    listening		varchar2( 1 )	not null,
    ignore_tracking	varchar2( 1 )	not null,
    source		varchar2( 255 )	not null,
    primary key( name ),
    constraint ml_l_dn foreign key( device_name ) references ml_device ( device_name )
)
/

create table ml_sis_sync_state (
    remote_id		varchar2( 128 ) not null,
    subscription_id	varchar2( 128 ) not null,
    publication_name	varchar2( 128 ) not null,
...
create procedure ml_set_device(
    ...
    p_listener_version	in varchar2,
    p_listener_protocol	in integer,
    p_info		in varchar2,
...
begin
    begin
	select device_name into d from ml_device where device_name = p_device;
    exception
    	when NO_DATA_FOUND then
	    d := NULL;
    end;
...
begin
    begin
	select device_name into d from ml_device_address
...
as
begin
    ml_set_device_address( p_device, p_medium, p_address, p_active, 'n', 'tracking' );
end;
/
...
create procedure ml_upload_update_listening(
    device		in	varchar2,
    listening		in	varchar2,
    name		in	varchar2 )
as
begin
    ml_set_listening( name, device, listening, 'n', 'tracking' );
end;
/
