...
-- a Sybase IQ consolidated database.
--
...
create table ml_ldap_server (
    ...
    primary key ( ldsrv_id ) ) IN SYSTEM
go

create table ml_trusted_certificates_file (
    file_name		varchar(1024) not null ) IN SYSTEM 
go
...
create table ml_user_auth_policy (
    ...
    primary key( policy_id ) ) IN SYSTEM 
go
...
create table ml_subscription (
    ...
    not null foreign key references ml_user ) IN SYSTEM
go

create table ml_table (
    table_id	    integer		not null default autoincrement,
    name	    varchar( 128 )     	not null,
    unique( name ),
    primary key( table_id ) ) IN SYSTEM
go
...
create table ml_script (
    ...
    ,
    primary key( script_id ) ) IN SYSTEM
go

create table ml_script_version (
    version_id		integer		not null default autoincrement,
    name		varchar( 128 )	not null,
    description		text		null,
    unique( name ),
    primary key( version_id ) ) IN SYSTEM
go
...
create table ml_connection_script (
    ...
    not null foreign key references ml_script,
    primary key( version_id, event ) ) IN SYSTEM
go
...
create table ml_table_script (
    ...
    not null foreign key references ml_script_version,
    not null foreign key references ml_script,
    primary key( version_id, table_id, event ) ) IN SYSTEM
go
...
create table ml_property (
    ...
    primary key( component_name, property_set_name, property_name ) ) IN SYSTEM
go

create table ml_scripts_modified (
    last_modified	datetime	not null primary key ) IN SYSTEM
go
...
create table ml_column (
    ...
    unique( version_id, table_id, name ) ) IN SYSTEM
go
...
create table ml_primary_server (
    ...
    unique( name ),
    primary key( server_id ) ) IN SYSTEM
go
...
create table ml_model_schema (
    ...
    locked		bit,
    primary key( schema_type, schema_owner, table_name, object_name ) ) IN SYSTEM
go
...
create table ml_model_schema_use (
    ...
    object_name		varchar( 128 )  not null,
    checksum		varchar( 64 )   not null,
    primary key( version_id, schema_type, schema_owner, table_name, object_name ) ) IN SYSTEM
go
...
create table ml_passthrough (
    ...
    foreign key( script_id ) references ml_passthrough_script( script_id ) ) IN SYSTEM
go
...
create table ml_passthrough_status (
    ...
    foreign key( remote_id ) references ml_database( remote_id ) ) IN SYSTEM
go
...
create table ml_passthrough_repair (
    ...
    foreign key( failed_script_id ) references ml_passthrough_script( script_id ) ) IN SYSTEM
go
...
create table ml_device (
    ...
    source		varchar( 255 )	not null ) IN SYSTEM
go
...
create table ml_device_address (
    ...
    primary key( device_name, medium ) ) IN SYSTEM
go
...
create table ml_sis_sync_state (
    ...
    primary key( remote_id, subscription_id ) ) IN SYSTEM
go
