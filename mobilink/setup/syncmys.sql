...
create table ml_database (
    ...
    script_ldt		    datetime       default '1900-01-01 00:00:00.00000' not null,
...
create table ml_subscription (
    ...
    last_upload_time	    datetime	    default '1900-01-01 00:00:00.00000' not null,
    last_download_time	    datetime	    default '1900-01-01 00:00:00.00000' not null,
...
insert into ml_scripts_modified ( last_modified ) values ( sysdate() )
//
...
create trigger ml_script_trigger_insert before
    insert on ml_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//

create trigger ml_script_trigger_update before
    update on ml_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//

create trigger ml_script_trigger_delete before
    delete on ml_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//

create trigger ml_table_script_trigger_insert before 
    insert on ml_table_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//
    
create trigger ml_table_script_trigger_update before 
    update on ml_table_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//
    
create trigger ml_table_script_trigger_delete before 
    delete on ml_table_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//

create trigger ml_connection_script_trigger_insert before 
    insert on ml_connection_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//

create trigger ml_connection_script_trigger_update before 
    update on ml_connection_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//

create trigger ml_connection_script_trigger_delete before 
    delete on ml_connection_script for each row
begin
    update ml_scripts_modified set last_modified = sysdate();
end;
//
...
create table ml_passthrough_repair (
    ...
    foreign key( new_script_id ) references ml_passthrough_script( script_id ) )
//

create procedure ml_add_passthrough_script (
    p_script_name	varchar( 128 ),
    p_flags		varchar( 256 ),
    p_affected_pubs	mediumtext,
    p_script		mediumtext,
...
begin
	...
	if p_flags is not null then
		...
		if v_substr is not null and length( v_substr ) > 0 then
			...
			set error = CONCAT( 'Invalid flag: "', v_substr, '"' );
			set v_error = 1;
			leave myr;
		    end if;
		end if;
		...
		set v_str = substring( v_str, v_end + 1, length( v_str ) - v_end );
	    end loop myr;
	end if;
	if v_error = 0 then
	    if not exists ( select * from ml_passthrough_script where script_name = p_script_name ) then
		...
		set error = CONCAT( 'The script name "', p_script_name, '" already exists in the ml_passthrough_script table.  Please choose another script name.' );
	    end if;
	end if;
    else
	-- this is a workaround because we are not able to raise an error in MySQL
	set error = 'Neither passthrough script name nor script content can be null.';
    end if;
end;
//
...
begin
    ...
    if v_script_id is not null then
        if not exists ( select * from ml_passthrough
			    where script_id = v_script_id ) and
	   not exists ( select * from ml_passthrough_repair
			    where failed_script_id = v_script_id ) and
	   not exists ( select * from ml_passthrough_repair
			    where new_script_id = v_script_id ) then
...
begin
	...
	if p_run_order is not null and p_run_order < 0 then
	    set error = 'A negative value for run_order is not allowed';
	else
	    if p_remote_id is null then
		begin
		    declare continue handler for SQLSTATE '02000' set done = 1;
		    if p_run_order is null then
			select coalesce( max( run_order ) + 10, 10 ) into v_order 
			    from ml_passthrough;
		    else
		...
		if p_run_order is null then
		    ...
		    if exists (select * from ml_passthrough
			       where remote_id = p_remote_id and run_order = p_run_order) then
	...
	set v_msg = CONCAT( 'Passthrough script name: "', v_name,
...
begin
	...
	if p_run_order is null then
	    ...
	    delete from ml_passthrough
		where run_order = p_run_order and script_id =
		    (select script_id from ml_passthrough_script where script_name = p_script_name);
	end if;
    else
	if p_run_order is null then
	    delete from ml_passthrough
		where remote_id = p_remote_id and script_id =
		    (select script_id from ml_passthrough_script where script_name = p_script_name);
	else
	    delete from ml_passthrough
		where remote_id = p_remote_id and run_order = p_run_order and script_id =
		    (select script_id from ml_passthrough_script where script_name = p_script_name);
	end if;
    end if;
end;
...
create procedure ml_add_passthrough_repair (
    p_failed_script_name	varchar( 128 ),
    p_error_code		integer,
    p_new_script_name		varchar( 128 ),
    p_action			char( 1 ) )
begin
    declare v_failed_script_id integer;
    ...
    if v_failed_script_id is not null then
		...
		set error = CONCAT( 'The new_script_name cannot be null for action "', p_action, '".' );
	    elseif p_action in ( 'S', 'P', 'H', 's', 'p', 'h' ) and p_new_script_name is not null then
		set error = CONCAT( 'The new_script_name should be null for action "', p_action, '".' );
		...
		set error = CONCAT( 'Invalid new_script_name: "', p_new_script_name, '".' );
	    else
	    ...
	    -- this is a workaround because we are not able to raise an error in MySQL			    
	    set error = CONCAT( 'Invalid action: "', v_name, '".' );
	end if;
    else
	if p_failed_script_name is not null then
	    set v_name = p_failed_script_name;
	else
	    set v_name = 'NULL';
	end if;
	-- this is a workaround because we are not able to raise an error in MySQL			    
	set error = CONCAT( 'Invalid failed_script_name: "', v_name, '".');
    end if;
end;
//
...
begin
    ...
    select version_id into v_version_id from ml_script_version 
        where name = p_version;
    select table_id into v_table_id from ml_table where name = p_table;
...
create procedure ml_add_lang_table_script (
    ...
    p_script		mediumtext )
begin
    call ml_add_lang_table_script_chk( p_version, p_table, p_event, p_script_language, p_script, null );
end;
//
...
create procedure ml_add_lang_connection_script (
    ...
    p_script		mediumtext )
begin
    call ml_add_lang_conn_script_chk( p_version, p_event, p_script_language, p_script, null );
end;
//
...
create procedure ml_add_property (
    p_comp_name		varchar( 128 ),
    p_prop_set_name	varchar( 128 ),
    p_prop_name		varchar( 128 ),
    p_prop_value	mediumtext )
begin
    declare 	p_name 	varchar( 128 );

    if p_prop_value is null then
	delete from ml_property where component_name  = p_comp_name
				and property_set_name = p_prop_set_name
				and property_name     = p_prop_name;
    else
...
create procedure ml_set_device(
    p_device		varchar( 255 ),
    p_listener_version	varchar( 128 ),
    p_listener_protocol	integer,
    p_info		varchar( 255 ),
    p_ignore_tracking	varchar( 1 ),
    p_source		varchar( 255 ) )
begin
    if not exists( select * from ml_device where device_name = p_device ) then
...
begin
    if not exists( select * from ml_device_address
		where device_name = p_device and medium = p_medium ) then
...
create procedure ml_set_listening(
    p_name		varchar( 128 ),
    p_device		varchar( 255 ),
    p_listening		varchar( 1 ),
    p_ignore_tracking	varchar( 1 ),
    p_source		varchar( 255 ) )
begin
    if not exists( select * from ml_listening where name = p_name ) then
...
create procedure ml_set_sis_sync_state(
    p_remote_id		varchar( 128 ),
    p_subscription_id	varchar( 128 ),
    p_publication_name	varchar( 128 ),
    p_user_name		varchar( 128 ),
    p_last_upload	datetime,
...
begin
	...
	set sid = concat( 's:', p_publication_name );
    else 
...
create procedure ml_delete_listening( p_name varchar( 128 ) )
begin
    delete from ml_listening where name = p_name;
end
//

create procedure ml_delete_device( p_device varchar( 255 ) )
begin
