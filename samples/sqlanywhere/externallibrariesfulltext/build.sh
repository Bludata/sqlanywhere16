...
#	SQLANY16	location of the SQL Anywhere installation
#				(default: /opt/sqlanywhere16)
...
INCDIR="${SAROOT}/sdk/include"
CFLAGS="$CFLAGS -I. -I${INCDIR}"
CXXFLAGS="${_CXXFLAGS:-}"

LIBDIRS="-L${SADIR}/${NATIVE_SA_LIBDIR}"
ODBC_LIBDIRS="-L. -L${ODBC}/lib"
...
$CXX $CFLAGS $LDFLAGS $SOFLAGS pf_sample.cpp -g -o $OBJDIR/libpf_sample.$SOEXT
$CXX $CFLAGS $LDFLAGS $SOFLAGS tb_sample.cpp -g -o $OBJDIR/libtb_sample.$SOEXT

if [ "${SED:-}" = "" ]; then
    echo "sed(1) is not installed. Edit pf_sample.sql and replace all instances of pf_sample.dll with libpf_sample.${SOEXT}. Also edit tb_sample.sql and replace all instances of tb_sample.dll with libtb_sample.${SOEXT}."
else
    $SED "s/pf_sample\.dll/libpf_sample\.$SOEXT/" pf_sample.sql > pf_sample.sql.tmp
    $SED "s/tb_sample\.dll/libtb_sample\.$SOEXT/" tb_sample.sql > tb_sample.sql.tmp
    mv pf_sample.sql.tmp pf_sample.sql
    mv tb_sample.sql.tmp tb_sample.sql
fi
