...
$CXX $CFLAGS $LDFLAGS $SOFLAGS extproc.cpp -g -o $OBJDIR/libextproc.$SOEXT

if [ "${SED:-}" = "" ]; then
    echo "sed(1) is not installed. Edit extproc.sql and replace all instances of extproc.dll with libextproc.${SOEXT}."
else
    $SED "s/extproc\.dll/libextproc\.$SOEXT/" extproc.sql > extproc.sql.tmp
    mv extproc.sql.tmp extproc.sql
fi
