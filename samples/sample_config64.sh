...
dbdsn -y -w "SQL Anywhere 16 Demo" -c "UID=DBA;PWD=sql;DBF="$SAMPLES_DESTDIR/sqlanywhere/demo.db";SERVERNAME=demo16_$USER;START="$SQLANY16/bin64/dbeng16";Description=SQL Anywhere 16 Sample Database"

dbdsn -y -w "SQL Anywhere 16 CustDB" -c "UID=ml_server;PWD=sql;DBF="$SAMPLES_DESTDIR/ultralite/custdb/custdb.db";SERVERNAME=custdb16_$USER;START="$SQLANY16/bin64/dbeng16";Description=SQL Anywhere 16 CustDB"
...
echo "Setting up sample_env script... "
    ...
    echo "# Set up the SQLANYSAMP16 env var for samples that may need"		    >> "$SAMPLES_DESTDIR/sample_env64.sh"
    echo "SQLANYSAMP16=$SAMPLES_DESTDIR"		>> "$SAMPLES_DESTDIR/sample_env64.sh"
    echo "export SQLANYSAMP16"		    >> "$SAMPLES_DESTDIR/sample_env64.sh"
